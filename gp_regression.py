...
class _base_gp():
    ...
    def _calc_misc_for_prediction(self, X_pred):
        ...
        i_Lambda = None
        KQ_ss = None
        return K_us, K_su, i_K_uu, Sigma, K_uf, i_Lambda, KQ_ss
    
    def predict(self, X_pred):
        K_us, K_su, i_K_uu, Sigma, K_uf, i_Lambda, KQ_ss = self._calc_misc_for_prediction(X_pred)
        y = self.y_train
        mean = K_su @ Sigma @ K_uf @ i_Lambda @ y
        cov = (KQ_ss - K_su @ i_K_uu @ K_us) + K_su @ Sigma @ K_us
        return mean, cov
    ...
    def make_params_df(self):
        ...
        return pd.DataFrame(self.params_opt_results, columns=col_name)
...
class Sparse_GP_regression_SoR(_base_sparse_gp):
...
class Sparse_GP_regression_DTC(_base_sparse_gp):
    def __init__(self, X_train, y_train, pseudo_inputs=None, p_inputs_num=None, p_optimized=False, kernel=None, noise=1.):
        super().__init__(X_train, y_train, pseudo_inputs, p_inputs_num, p_optimized, kernel, noise)
...
class Sparse_GP_regression_FITC(_base_sparse_gp):
    def __init__(self, X_train, y_train, pseudo_inputs=None, p_inputs_num=None, p_optimized=False, kernel=None, noise=1.):
        super().__init__(X_train, y_train, pseudo_inputs, p_inputs_num, p_optimized, kernel, noise)
